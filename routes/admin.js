...
router.delete('/cheese-boards/:id', adminAuth, async (req, res) => {
  try {
    ...
    // Delete all items for this board
    await MenuItem.deleteMany({ category: 'cheese-board', pageOption: board.type });
    
    // Delete subcategory limits
    await SubcategoryLimit.deleteMany({ page: 'cheese-boards', pageVariant: board.type });
